      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="md:col-span-2">
          ...
          <input
            ...
            className="w-full px-3 py-2.5 bg-foreground/[0.04] border border-foreground/[0.08] rounded-lg
                       text-sm text-foreground/90 placeholder:text-faint
        ...
        <div className="flex items-end pb-1">
          <label className="flex items-center gap-2.5 cursor-pointer group">
            <div className="relative">
              ...
              <div className="absolute left-0.5 top-0.5 w-4 h-4 bg-foreground/40 rounded-full shadow
                              peer-checked:translate-x-4 peer-checked:bg-cyan-400 transition-all" />
            </div>
            <span className="text-xs text-muted group-hover:text-muted transition-colors">
        ...
        <div className={`grid gap-4 ${showPreview ? 'grid-cols-1 lg:grid-cols-2' : 'grid-cols-1'}`}>
          ...
          <div>
            <textarea
              ...
              className="w-full h-80 px-4 py-3 bg-foreground/[0.03] border border-foreground/[0.08] rounded-lg
                         text-sm text-foreground/80 font-mono leading-relaxed placeholder:text-faint
                         focus:outline-none focus:border-cyan-500/40 focus:ring-1 focus:ring-cyan-500/20
          ...
          {/* Preview pane */}
          {showPreview && (
            <div className="h-80 overflow-y-auto rounded-lg bg-foreground/[0.02] border border-foreground/[0.06] p-4">
              {content.trim() ? (
                <MarkdownRenderer content={content} />
              ) : (
                <p className="text-sm text-faint italic">La vista previa aparecerá aquí...</p>
              )}
            </div>
          )}
        </div>
